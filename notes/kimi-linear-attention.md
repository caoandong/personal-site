...
To support further research, we open-source the KDA kernel and vLLM implementations<sup>1</sup>, and release the pre-trained and instruction-tuned model checkpoints.<sup>2</sup>

![Figure 1: (a) Performance vs. acceleration. (b) Time per output token (TPOT) vs. decoding length.](1066b22ea190b81c91c3441ea7618236_img.jpg)

Figure 1 consists of two plots. Plot (a) shows Performance (Y-axis, 45 to 60) versus Decoding Acceleration (X-axis, 1x to 4x). It compares Kimi Linear (red star), MLA (blue circle), GDN-H (blue circle), and Kimi Linear (red star). Kimi Linear achieves the highest performance (51.0) at 1x acceleration. At 4x acceleration, Kimi Linear (84.3) and MLA (81.3) are Pareto-optimal. Plot (b) shows TPOT (ms) (Y-axis, 0 to 10) versus Decoding Length (X-axis, 4K to 1M). It compares MLA (dashed green line), GDN-H (solid red line), and Kimi Linear (solid blue line). Kimi Linear maintains a low TPOT, matching GDN-H and outperforming MLA at long sequences. The plot shows that Kimi Linear is 6.3x faster than MLA at 1M tokens.

| Model       | Decoding Acceleration | Performance |
|-------------|-----------------------|-------------|
| Kimi Linear | 1x                    | 51.0        |
| GDN-H       | 1x                    | 47.9        |
| MLA         | 1x                    | 47.2        |
...
| Decoding Length | MLA (ms) | GDN-H (ms) | Kimi Linear (ms) |
|-----------------|----------|------------|------------------|
| 4K              | ~0.1     | ~0.1       | ~0.1             |
| 128K            | ~1.0     | ~1.0       | ~1.0             |
| 256K            | ~2.0     | ~2.0       | ~2.0             |
...
Figure 1: (a) Performance vs. acceleration. (b) Time per output token (TPOT) vs. decoding length.
...
<sup>1</sup> <https://github.com/fla-org/flash-linear-attention/tree/main/fla/ops/kda>

<sup>2</sup> <https://huggingface.co/moonshotai/Kimi-Linear-48B-A3B-Instruct>
...
In this paper, we define  $\square_t \in \mathbb{R}^{d_k}$  or  $\mathbb{R}^{d_v}$ , s. t.,  $\square \in \{q, k, v, o, u, w\}$  denotes a  $t$ -th corresponding column vector, and  $S_t \in \mathbb{R}^{d_k \times d_v}$  represents the matrix-form memory state.  $M$  and  $M^*$  denote lower-triangular masks with and without diagonal elements, respectively; for convenience, we also write them as Tril and StrictTril.

**Chunk-wise Formulation** Suppose the sequence is split into  $L/C$  chunks where each chunk is of length  $C$ . We define  $\square_{[t]} \in \mathbb{R}^{C \times d}$  for  $\square \in \{Q, K, V, O, U, W\}$  are matrices that stack the vectors within the  $t$ -th chunk, and  $\square_{[t]}^r = \square_{tC+r}$  is the  $r$ -th element of the chunk. Note that  $t \in [0, L/C]$ ,  $r \in [1, C]$ . State matrices are also re-indexed such that  $S_{[t]}^r = S_{tC+i}$ . Additionally,  $S_{[t]} := S_{[t]}^0 = S_{[t-1]}^C$ , i.e., the initial state of a chunk is the last state of the previous chunk.

**Decay Formulation** We define the cumulative decay  $\gamma_{[t]}^{i \rightarrow j} := \prod_{k=i}^j \alpha_{[t]}^k$ , and abbreviate  $\gamma_{[t]}^{1 \rightarrow r}$  as  $\gamma_{[t]}^r$ . Additionally,  $A_{[t]} := A_{[t]}^{i/j} \in \mathbb{R}^{C \times C}$  is the matrix with elements  $\gamma_{[t]}^i / \gamma_{[t]}^j$ .  $\text{Diag}(\alpha_t)$  denotes the fine-grained decay,  $\text{Diag}(\gamma_{[t]}^{i \rightarrow j}) := \prod_{k=i}^j \text{Diag}(\alpha_{[t]}^k)$ , and  $\Gamma_{[t]}^{i \rightarrow j} \in \mathbb{R}^{C \times d_k}$  is the matrix stack from  $\gamma_{[t]}^i$  to  $\gamma_{[t]}^j$ .
...
From the fast-weight perspective [84, 85],  $S_t$  serves as an associative memory storing transient mappings from keys to values. This update can be viewed as performing gradient *descent* on the unbounded correlation objective
...
This rule—the classical *delta rule*—treats  $S$  as a learnable associative memory that continually corrects itself toward the mapping  $k_t \mapsto v_t$ . The rank-1 update structure, equivalent to a generalized Householder transformation, supports hardware-efficient chunkwise parallelization [11, 112].
...
<sup>3</sup>When the state transformation matrix preserves its orthogonality, absolute positional encodings can also be applied independently to  $q$  and  $k$  to be converted into relative positional encodings during the attention computation [87].
...
Image: Diagram illustrating the chunkwise parallelization of the KDA recurrence. It shows a sequence of operations: a matrix S\_{t-1} (represented as a grid of blue and white squares) is multiplied by a diagonal matrix (represented as a grid of orange and white squares) to produce a new matrix. This is then added to a rank-1 update term (represented as a grid of orange and white squares) to produce the final matrix S\_t. The diagram uses color-coding to represent different components: blue for the main matrix, orange for the diagonal and update terms, and red for the gating matrix.
...
$$\mathbf{M}_{[t]} = \left( \mathbf{I} + \text{StrictTril} \left( \text{Diag}(\beta_{[t]}) \left( \mathbf{R}_{[t]}^{1 \rightarrow C} \odot \mathbf{K}_{[t]} \right) \left( \frac{\mathbf{K}_{[t]}}{\mathbf{R}_{[t]}^{1 \rightarrow C}} \right)^\top \right) \right)^{-1} \text{Diag}(\beta_{[t]}) \quad (6)$$

$$\mathbf{W}_{[t]} = \mathbf{M}_{[t]} \left( \mathbf{R}_{[t]}^{1 \rightarrow C} \odot \mathbf{K}_{[t]} \right), \quad \mathbf{U}_{[t]} = \mathbf{M}_{[t]} \mathbf{V}_{[t]} \quad (7)$$
...
$$\mathbf{S}_{[t+1]} = \text{Diag}(\gamma_{[t]}^C) \mathbf{S}_{[t]} + \left( \mathbf{R}_{[t]}^{1 \rightarrow C} \odot \mathbf{K}_{[t]} \right)^\top (\mathbf{U}_{[t]} - \mathbf{W}_{[t]} \mathbf{S}_{[t]}) \in \mathbb{R}^{d_k \times d_v} \quad (8)$$

$$\begin{bmatrix} \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \end{bmatrix} = \begin{bmatrix} \text{red} & & & \\ & \text{red} & & \\ & & \text{red} & \\ & & & \text{red} \end{bmatrix} \begin{bmatrix} \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \end{bmatrix} + \left( \begin{bmatrix} \text{red} & \text{red} & \text{red} & \text{red} \\ \text{red} & \text{red} & \text{red} & \text{red} \\ \text{red} & \text{red} & \text{red} & \text{red} \\ \text{red} & \text{red} & \text{red} & \text{red} \end{bmatrix} \odot \begin{bmatrix} \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \end{bmatrix} \right) \begin{bmatrix} \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \end{bmatrix}$$
...
$$\mathbf{O}_{[t]} = \underbrace{\left( \mathbf{\Gamma}_{[t]}^{1 \rightarrow C} \odot \mathbf{Q}_{[t]} \right) \mathbf{S}_{[t]}}_{\text{inter chunk}} + \underbrace{\text{Tril} \left( \left( \mathbf{\Gamma}_{[t]}^{1 \rightarrow C} \odot \mathbf{Q}_{[t]} \right) \left( \frac{\mathbf{K}_{[t]}}{\mathbf{\Gamma}_{[t]}^{1 \rightarrow C}} \right)^{\top} \right)}_{\text{intra chunk}} \underbrace{\left( \mathbf{U}_{[t]} - \mathbf{W}_{[t]} \mathbf{S}_{[t]} \right)}_{\text{"pseudo"-value term}} \in \mathbb{R}^{C \times d_v} \quad (9)$$

$$\begin{bmatrix} \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \end{bmatrix} = \left( \begin{bmatrix} \text{red} & \text{red} & \text{red} & \text{red} \\ \text{red} & \text{red} & \text{red} & \text{red} \\ \text{red} & \text{red} & \text{red} & \text{red} \\ \text{red} & \text{red} & \text{red} & \text{red} \end{bmatrix} \odot \begin{bmatrix} \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \end{bmatrix} \right) \begin{bmatrix} \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \\ \text{blue} & \text{blue} & \text{blue} & \text{blue} \end{bmatrix} + \begin{bmatrix} \text{red} & & & \\ & \text{red} & & \\ & & \text{red} & \\ & & & \text{red} \end{bmatrix} \begin{bmatrix} \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \\ \text{orange} & \text{orange} & \text{orange} & \text{orange} \end{bmatrix}$$
...
![Figure 2: Execution time of kernels for varying input lengths. The graph plots Execution Time (ms) on the y-axis (0 to 64) against Input length on the x-axis (2K, 4K, 8K, 16K, 32K, 64K). Two series are shown: DPLR (green dotted line with diamond markers) and KDA (ours) (blue solid line with triangle markers). DPLR shows a sharp increase in execution time as input length increases, reaching approximately 60 ms at 64K. KDA shows a much more gradual increase, reaching approximately 32 ms at 64K.](bc04d002fc79e8a3637b1552ac3361e6_img.jpg)

| Input length | DPLR (ms) | KDA (ms) |
|--------------|-----------|----------|
| 2K           | ~2        | ~1       |
| 4K           | ~4        | ~2       |
| 8K           | ~8        | ~4       |
...
Figure 2: Execution time of kernels for varying input lengths. The graph plots Execution Time (ms) on the y-axis (0 to 64) against Input length on the x-axis (2K, 4K, 8K, 16K, 32K, 64K). Two series are shown: DPLR (green dotted line with diamond markers) and KDA (ours) (blue solid line with triangle markers). DPLR shows a sharp increase in execution time as input length increases, reaching approximately 60 ms at 64K. KDA shows a much more gradual increase, reaching approximately 32 ms at 64K.
...
$$\begin{aligned} \mathbf{q}_t^h, \mathbf{k}_t^h &= \text{L2Norm}(\text{Swish}(\text{ShortConv}(\mathbf{W}_{q/k}^h \mathbf{x}_t))) \in \mathbb{R}^{d_k} \\ \mathbf{v}_t^h &= \text{Swish}(\text{ShortConv}(\mathbf{W}_v^h \mathbf{x}_t)) \in \mathbb{R}^{d_v} \\ \alpha_t^h &= f(\mathbf{W}_\alpha^h \mathbf{W}_\alpha^{\top} \mathbf{x}_t) \in [0, 1]^{d_k} \\ \beta_t^h &= \text{Sigmoid}(\mathbf{W}_\beta^h \mathbf{x}_t) \in [0, 1] \end{aligned}$$

where  $d_k, d_v$  represent the key and value head dimensions, which are set to 128 for all experiments. For  $\mathbf{q}, \mathbf{k}, \mathbf{v}$ , we apply a ShortConv followed by a Swish activation, following [111]. The  $\mathbf{q}$  and  $\mathbf{k}$  representations are further normalized using L2Norm to ensure eigenvalues stability, as suggested by [112]. The per-channel decay  $\alpha_t^h$  is parameterized via a low-rank projection ( $\mathbf{W}_\alpha^h$  and  $\mathbf{W}_\alpha^{\top}$  with rank equal to the head dimension) and a decay function  $f(\cdot)$  similar to those

![Figure 3: Illustration of the Kimi Linear model architecture. The diagram shows a stack of blocks. The top block (1x) contains a MoE layer, a Norm layer, an MLA layer, and another Norm layer. The bottom block (N x) contains a MoE layer, a Norm layer, a KDA layer, and another Norm layer. A detailed view of the MoE layer shows it routing inputs through N_s shared experts (green boxes) and N_r routed experts (blue boxes) using a Router. A detailed view of the KDA layer shows it takes inputs from Linear, Conv, and L2 layers, processes them through Kimi Delta Attention, and then through Norm and Linear layers.](a7d78d22e465dea388b31d0739f9d0cd_img.jpg)

Figure 3: Illustration of the Kimi Linear model architecture. The diagram shows a stack of blocks. The top block (1x) contains a MoE layer, a Norm layer, an MLA layer, and another Norm layer. The bottom block (N x) contains a MoE layer, a Norm layer, a KDA layer, and another Norm layer. A detailed view of the MoE layer shows it routing inputs through N\_s shared experts (green boxes) and N\_r routed experts (blue boxes) using a Router. A detailed view of the KDA layer shows it takes inputs from Linear, Conv, and L2 layers, processes them through Kimi Delta Attention, and then through Norm and Linear layers.
...
$$\mathbf{o}_t = \mathbf{W}_o (\text{Sigmoid}(\mathbf{W}_g^\top \mathbf{W}_v^\dagger \mathbf{x}_t) \odot \text{RMSNorm}(\text{KDA}(\mathbf{q}_t, \mathbf{k}_t, \mathbf{v}_t, \boldsymbol{\alpha}_t, \beta_t))) \quad (10)$$
...
![Figure 4: Results on synthetic tasks: palindrome, multi query associative recall, and the state tracking. The figure consists of six line plots arranged in a 2x3 grid. The top row shows Accuracy (%) vs. Sequence length (256, 512, 1024, 2048) for (a) Palindrome, (b) MQAR, and (c) Stack. The bottom row shows Accuracy (%) vs. Training steps (5K, 10K, 15K, 20K) for the same tasks. Three methods are compared: KDA (blue solid line with circles), GDN (green dashed line with stars), and Mamba2 (orange dashed line with triangles). In all cases, KDA maintains high accuracy (near 100%) across all sequence lengths and training steps. GDN and Mamba2 show significant drops in accuracy as sequence length increases, especially for the Stack task.](c0843c6d138705289960d9f53a6e72a1_img.jpg)
...
|               |        |        |        |        |        |        |        |        |   |   |   |   |   |   |        |
|---------------|--------|--------|--------|--------|--------|--------|--------|--------|---|---|---|---|---|---|--------|
| <b>Input</b>  | O      | G      | R      | S      | U      | N      | E      | <SEP>  | E | N | U | S | R | G | O      |
| <b>Output</b> | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | N | U | S | R | G | O | $\phi$ |
...
|               |        |        |        |        |        |        |        |        |        |        |        |        |        |   |   |
|---------------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|---|---|
| <b>Input</b>  | A      | 1      | C      | 3      | B      | 0      | M      | 8      | G      | 5      | E      | 4      | <SEP>  | B | G |
| <b>Output</b> | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | $\phi$ | 0 | 5 |
...
| # Act. Params. <sup>†</sup> | Head | Layer | Hidden | Tokens | Lr                     | batch size <sup>‡</sup> |
|-----------------------------|------|-------|--------|--------|------------------------|-------------------------|
...
<sup>‡</sup> All models were trained with a context length of 4,096.

![Figure 5: A log-log plot showing the fitted scaling law curves for MLA and Kimi Linear. The x-axis is PFLOP/s-days (log scale, 10^1 to 10^2) and the y-axis is Loss (linear scale, 2.0 to 2.2). The MLA curve is a dashed blue line with the equation 2.3092 * C^-0.0536. The Kimi Linear curve is a dashed red line with the equation 2.2879 * C^-0.0527. A horizontal arrow indicates a 1.16x difference in PFLOP/s-days between the two curves at a loss of approximately 2.08.](0f3e3ea50bcceb86f6c524ab2b6f3e7a_img.jpg)

Figure 5: A log-log plot showing the fitted scaling law curves for MLA and Kimi Linear. The x-axis is PFLOP/s-days (log scale, 10^1 to 10^2) and the y-axis is Loss (linear scale, 2.0 to 2.2). The MLA curve is a dashed blue line with the equation 2.3092 \* C^-0.0536. The Kimi Linear curve is a dashed red line with the equation 2.2879 \* C^-0.0527. A horizontal arrow indicates a 1.16x difference in PFLOP/s-days between the two curves at a loss of approximately 2.08.
...
- **Code Generation:** LiveCodeBench v6 <sup>4</sup>[44], EvalPlus [60].
- **Math & Reasoning:** AIME 2025, MATH 500, HMMT 2025, PolyMath-en.
- **Long-context:** MRCR <sup>5</sup>, RULER [38], Frames [52], HELMET-ICL [118], RepoQA [61], Long Code Arena [13] and LongBench v2 [6].
- **Chinese Language Understanding and Reasoning:** C-Eval [43], and CMMMLU [55].
...
<sup>4</sup>Questions from 2024.8 to 2025.5

<sup>5</sup><https://huggingface.co/datasets/openai/mrcr>
...
| <i>General</i>         | HellaSwag      | 81.7        | 82.2        | <b>82.9</b> |
|                        | ARC-challenge  | 64.6        | 66.5        | <b>67.3</b> |
|                        | Winogrande     | 78.1        | 77.9        | <b>78.6</b> |
|                        | BBH            | 71.6        | 70.6        | <b>72.9</b> |
|                        | MMLU           | 71.6        | 72.2        | <b>73.8</b> |
|                        | MMLU-Pro       | 47.2        | 47.9        | <b>51.0</b> |
|                        | TriviaQA       | 68.9        | 70.1        | <b>71.7</b> |
| <i>Math &amp; Code</i> | GSM8K          | 83.7        | 81.7        | <b>83.9</b> |
|                        | MATH           | <b>54.7</b> | 54.1        | <b>54.7</b> |
|                        | EvalPlus       | 59.5        | <b>63.1</b> | 60.2        |
|                        | CRUXEval-I-cot | 51.6        | 56.0        | <b>56.6</b> |
|                        | CRUXEval-O-cot | 61.5        | 58.1        | <b>62.0</b> |
| <i>Chinese</i>         | CEval          | 79.3        | 79.1        | <b>79.5</b> |
|                        | CMMLU          | 79.5        | 80.7        | <b>80.8</b> |
...
|------------------------|---------------------------|-------------|-------------|-------------|
|                        | Trained Tokens            | 1.4T        | 1.4T        | 1.4T        |
| <i>General</i>         | BBH                       | 68.2        | 68.5        | <b>69.4</b> |
|                        | MMLU                      | 75.7        | 75.6        | <b>77.0</b> |
|                        | MMLU-Pro                  | 65.7        | 64.8        | <b>67.4</b> |
|                        | MMLU-Redux                | 79.2        | 78.7        | <b>80.3</b> |
|                        | GPQA-Diamond (Avg@8)      | 57.1        | 58.6        | <b>62.1</b> |
|                        | LiveBench (Pass@1)        | 45.7        | <b>46.4</b> | 45.2        |
| <i>Math &amp; Code</i> | AIME 2025 (Avg@64)        | 20.6        | 21.1        | <b>21.3</b> |
|                        | MATH500 (Acc.)            | 80.8        | <b>83.0</b> | 81.2        |
|                        | HMMT 2025 (Avg@32)        | 11.3        | 11.3        | <b>12.5</b> |
|                        | PolyMath-en (Avg@4)       | 41.3        | 41.5        | <b>43.6</b> |
|                        | LiveCodeBench v6 (Pass@1) | 25.1        | 25.4        | <b>26.0</b> |
|                        | EvalPlus                  | <b>62.6</b> | 62.5        | 61.0        |
...
![Figure 6: Training and test accuracy curves for Kimi Linear@1.4T and MLA@1.4T. (a) Training accuracy on Math RL training set. (b) Test accuracy on MATH 500 Test. (c) Test accuracy on AIME 2025. In all cases, Kimi Linear (solid blue line) outperforms MLA (dashed green line).](252ea48d02dce93965b91746fb376f35_img.jpg)

Figure 6 consists of three line graphs labeled (a), (b), and (c). Each graph plots Accuracy (Y-axis) against Train or Test steps (X-axis). The legend for all graphs is: MLA@1.4T (dashed green line with circles) and Kimi Linear@1.4T (solid blue line with circles).

- (a) Training accuracy: The X-axis is 'Train' (0 to 100). The Y-axis is 'Accuracy' (20 to 65). Kimi Linear starts at ~35 and rises to ~60. MLA starts at ~20 and rises to ~50.
- (b) Test accuracy on MATH 500 Test: The X-axis is 'MATH 500 Test' (0 to 100). The Y-axis is 'Accuracy' (70 to 94). Kimi Linear starts at ~75 and rises to ~88. MLA starts at ~70 and rises to ~85.
- (c) Test accuracy on AIME 2025: The X-axis is 'AIME 2025' (0 to 100). The Y-axis is 'Accuracy' (10 to 25). Kimi Linear starts at ~12 and rises to ~22. MLA starts at ~10 and rises to ~18.

Figure 6: Training and test accuracy curves for Kimi Linear@1.4T and MLA@1.4T. (a) Training accuracy on Math RL training set. (b) Test accuracy on MATH 500 Test. (c) Test accuracy on AIME 2025. In all cases, Kimi Linear (solid blue line) outperforms MLA (dashed green line).
...
**Prefilling & Decoding speed** We compare the training and decoding times for full attention MLA [19], GDN-H, and Kimi Linear in Figure 7a and Figure 7b. Note that all models are based on the Kimi Linear 48B setting, with the same number of layers and attention heads. We observe that: 1) Despite incorporating a more fine-grained decay mechanism, Kimi Linear introduces negligible latency overhead compared to GDN-H during prefiling. As shown in Figure 7a, their

![Figure 7: (a) Prefilling time of MLA, GDN-H, and Kimi Linear. (b) Time per output token (TPOT) for MLA, GDN-H, and Kimi Linear during decoding.](b05a8a3551db31147979064952179990_img.jpg)

Figure 7 consists of two line graphs, (a) and (b), comparing the performance of MLA, GDN-H, and Kimi Linear across different sequence lengths.

(a) Prefilling time (s) vs. Prefilling Length. The x-axis shows sequence lengths: 4K, 128K, 256K, 512K, and 1M. The y-axis shows latency in seconds, ranging from 0 to 60. MLA (dashed green line) shows a sharp increase in latency at 1M, reaching approximately 60s. GDN-H (dashed orange line) and Kimi Linear (solid blue line) show much lower latency, with Kimi Linear being significantly faster at 1M, reaching approximately 20s. Red arrows indicate speedup factors: 2.3x at 512K and 2.9x at 1M for Kimi Linear over GDN-H.

(b) TPOT (ms) vs. Decoding Length. The x-axis shows sequence lengths: 4K, 128K, 256K, 512K, and 1M. The y-axis shows TPOT in milliseconds, ranging from 5 to 15. MLA (dashed green line) shows a sharp increase in TPOT at 1M, reaching approximately 15ms. GDN-H (dashed orange line) and Kimi Linear (solid blue line) show much lower TPOT, with Kimi Linear being significantly faster at 1M, reaching approximately 8ms. Red arrows indicate speedup factors: 1.8x at 512K and 2.2x at 1M for Kimi Linear over GDN-H.

Figure 7: (a) Prefilling time of MLA, GDN-H, and Kimi Linear. (b) Time per output token (TPOT) for MLA, GDN-H, and Kimi Linear during decoding.
...
From this perspective, GDN can be interpreted as a form of multiplicative positional encoding whose transition matrix is data-dependent, thereby relaxing the orthogonality constraint imposed by RoPE and can be potentially more powerful [115].<sup>6</sup> This provides a potential solution to the known extrapolation issues of RoPE, whose fixed frequencies can cause overfitting to context lengths seen during training [108, 72]. Some recent works adopt workarounds like partial RoPE

<sup>6</sup>When preserving orthogonality, absolute positional encodings can be applied independently to  $\mathbf{q}$  and  $\mathbf{k}$ , which are then automatically transformed into relative positional encodings during the attention computation [87].

Table 6: An overview of attention mechanisms in their mathematically equivalent recurrent ( $\mathcal{O}_t$ ) and parallel ( $\mathcal{O}$ ) forms. We omitted the normalization term and  $\beta_t$  to achieve a more concise representation. The function  $\phi$  refers to the infinite-dimensional feature space corresponding to the exponential kernel, i.e.,  $\phi(\mathbf{q})^\top \phi(\mathbf{k}) = \exp(\mathbf{q}^\top \mathbf{k})$ .
...
| SA + RoPE [88] $\sum_{j=1}^t \exp\left(\mathbf{q}_t^\top \left(\prod_{s=j+1}^t \mathbf{R}_s\right) \mathbf{k}_j\right) \mathbf{v}_j$                                                                        | $(\exp(\mathbf{R}(\mathbf{Q} \odot \mathbf{R}) \mathbf{K}^\top) \odot \mathbf{M}) \mathbf{V}$                                                                                                                                                                                                   |
| LA [101] $\sum_{j=1}^t (\tilde{\mathbf{q}}_t^\top \mathbf{k}_j) \mathbf{v}_j$                                                                                                                               | $(\mathbf{QK}^\top \odot \mathbf{M}) \mathbf{V}$                                                                                                                                                                                                                                                |
| Mamba2 [16] $\sum_{j=1}^t \mathbf{q}_t^\top \left(\prod_{s=j+1}^t \alpha_s\right) \mathbf{k}_j \mathbf{v}_j$                                                                                                | $(\mathbf{QK}^\top \odot \mathcal{A} \odot \mathbf{M}) \mathbf{V}$                                                                                                                                                                                                                              |
| GLA [114] $\sum_{j=1}^t \mathbf{q}_t^\top \left(\prod_{s=j+1}^t \text{Diag}(\alpha_s)\right) \mathbf{k}_j \mathbf{v}_j$                                                                                     | $((\mathbf{Q} \odot \mathbf{F}) \left(\frac{\mathbf{K}}{\mathbf{P}}\right)^\top \odot \mathbf{M}) \mathbf{V}$                                                                                                                                                                                   |
| DeltaNet [84] $\sum_{j=1}^t \mathbf{q}_t^\top \left(\prod_{s=j+1}^t (\mathbf{I} - \mathbf{k}_s \mathbf{k}_s^\top)\right) \mathbf{k}_j \mathbf{v}_j$                                                         | $(\mathbf{QK}^\top \odot \mathbf{M}) (\mathbf{I} + \mathbf{KK}^\top \odot \mathbf{M}^{-1})^{-1} \mathbf{V}$                                                                                                                                                                                     |
...
| DeltaFormer [125] $\sum_{j=1}^t \left(\phi(\mathbf{q}_t)^\top \left(\prod_{s=j+1}^t (\mathbf{I} - \phi(\mathbf{k}_s) \phi(\mathbf{w}_s)^\top)\right) \phi(\mathbf{k}_j)\right) \mathbf{v}_j$                | $(\exp(\mathbf{QK}^\top) \odot \mathbf{M}) (\mathbf{I} + \exp(\mathbf{WK}^\top) \odot \mathbf{M}^{-1})^{-1} \mathbf{V}$                                                                                                                                                                         |
| PaTH-FoX [115] $\sum_{j=1}^t \exp\left(\mathbf{q}_t^\top \left(\prod_{s=j+1}^t (\mathbf{I} - \mathbf{w}_s \mathbf{w}_s^\top)\right) \mathbf{k}_j\right) \left(\prod_{s=j+1}^t \alpha_s\right) \mathbf{v}_j$ | $(\exp((\mathbf{QK}^\top \odot \mathbf{M}) (\mathbf{I} + \mathbf{WW}^\top \odot \mathbf{M}^{-1})^{-1}) \odot \mathcal{A} \odot \mathbf{M}) \mathbf{V}$                                                                                                                                          |
| GDN [111] $\sum_{j=1}^t \mathbf{q}_t^\top \left(\prod_{s=j+1}^t \alpha_s (\mathbf{I} - \mathbf{k}_s \mathbf{k}_s^\top)\right) \mathbf{k}_j \mathbf{v}_j$                                                    | $(\mathbf{QK}^\top \odot \mathcal{A} \odot \mathbf{M}) (\mathbf{I} + \mathbf{KK}^\top \odot \mathcal{A} \odot \mathbf{M}^{-1})^{-1} \mathbf{V}$                                                                                                                                                 |
| Comba [40] $\sum_{j=1}^t \mathbf{q}_t^\top \left(\prod_{s=j+1}^t (\alpha_s - \mathbf{k}_s \mathbf{k}_s^\top)\right) \mathbf{k}_j \mathbf{v}_j$                                                              | $(\mathbf{QK}^\top \odot \mathcal{A} \odot \mathbf{M}) (\mathbf{I} + \mathbf{KK}^\top \odot \mathcal{A}^{1-1/j} \odot \mathbf{M}^{-1})^{-1} \mathbf{V}$                                                                                                                                         |
| RWKv7 [71] $\sum_{j=1}^t \mathbf{q}_t^\top \left(\prod_{s=j+1}^t (\text{Diag}(\alpha_s) - (\mathbf{b}_s \odot \mathbf{k}_s) \mathbf{k}_s^\top)\right) \mathbf{k}_j \mathbf{v}_j$                            | $((\mathbf{Q} \odot \mathbf{F}) \left(\frac{\mathbf{K}}{\mathbf{P}}\right)^\top \odot \mathbf{M}) \left(\mathbf{I} + \left(\mathbf{K} \odot \frac{\mathbf{F}}{\mathbf{T}}\right) \left(\frac{\mathbf{K} \odot \mathbf{B}}{\mathbf{T}}\right)^\top \odot \mathbf{M}^{-1}\right)^{-1} \mathbf{V}$ |
| <b>KDA (ours)</b> $\sum_{j=1}^t \mathbf{q}_t^\top \left(\prod_{s=j+1}^t \text{Diag}(\alpha_s) (\mathbf{I} - \mathbf{k}_s \mathbf{k}_s^\top)\right) \mathbf{k}_j \mathbf{v}_j$                               | $((\mathbf{Q} \odot \mathbf{F}) \left(\frac{\mathbf{K}}{\mathbf{P}}\right)^\top \odot \mathbf{M}) (\mathbf{I} + (\mathbf{K} \odot \mathbf{F}) \left(\frac{\mathbf{K}}{\mathbf{P}}\right)^\top \odot \mathbf{M}^{-1})^{-1} \mathbf{V}$                                                           |
...
Furthermore, by sharing  $\alpha_t$ , we can factor it out as in Eq. 1, enabling a fine-grained multiplicative decay over  $\mathbf{S}_t$  in a manner similar to GLA [114], followed by a Householder-style transformation like DeltaNet [84, 112] for efficient
...
20         Aab[:, :, i, :] = Aab[:, :, i-1, :] + (Aab[:, :, i-1,
21             → None] * Aab[:, :, i-1, :]).sum(-2)
...
27         q_i, k_i, v_i, u_i, w_i, b_i = (x[:, :, :, i] for x in
28             (q, k, v, u, w, b))
29         o1 = Aqk[:, :, i, :] @ u_i
30         o2 = Aqb[:, :, i, :] @ w_i
...
30         decay = (g_i[:, :, :, i-1] - g_i).exp()
31         S = S * g_i[:, :, :, i-1, None].exp()
...
| Comba [40]    | $\frac{\beta_t}{2} \ \mathbf{S}_{t-1}^\top \mathbf{k}_t - \mathbf{v}_t\ ^2 + \frac{1}{2} \ \sqrt{1-\alpha_t} \mathbf{S}_{t-1}\ _F^2$                            | $\mathbf{S}_t = \left( \alpha_t - \beta_t \mathbf{k}_t \mathbf{k}_t^\top \right) \mathbf{S}_{t-1} + \beta_t \mathbf{k}_t \mathbf{v}_t^\top$                                                    |
| RWKV7 [71]    | $\frac{1}{2} \ \mathbf{S}_{t-1}^\top \mathbf{k}_t - \mathbf{v}_t\ ^2 + \frac{1}{2} \ \sqrt{\text{Diag}(\mathbf{1} - \alpha_t)} \mathbf{S}_{t-1}\ _F^2$          | $\mathbf{S}_t = \left( \text{Diag}(\alpha_t) - \left( \mathbf{b}_t \odot \hat{\mathbf{k}}_t \right) \hat{\mathbf{k}}_t^\top \right) \mathbf{S}_{t-1} + \mathbf{k}_t \mathbf{v}_t^\top$         |
...
For GDN and KDA, the update can be viewed as performing an Stochastic Gradient Descent(SGD) process on the decayed state  $\hat{\mathbf{S}}$ , that is,  $\mathbf{S}_t = \hat{\mathbf{S}}_{t-1} - \nabla_{\hat{\mathbf{S}}_{t-1}} \mathcal{L}$ , where  $\hat{\mathbf{S}}_{t-1}$  is decayed by scalar or fine-grained gate.
...
The primary distinction among various gated linear attention mechanisms lies in the parameterization of the forget gate  $\mathbf{A}_t$ , as summarized in Table 7. For instance, RetNet [92] uses a data-independent scalar decay  $\alpha$ , and Mamba2 [16] employs a data-dependent scalar  $\alpha_t$ . Specifically, GLA [114] utilized a diagonalized fine-grained matrix  $\text{Diag}(\alpha_t) \in \mathbb{R}^{d_k \times d_k}$ , offering an effective trade-off between efficiency and performance. Other variants are displayed in Table 7.
...
- [17] Tri Dao et al. “FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness”. In: *Advances in NeurIPS*. 2022, pp. 16344–16359. URL: [https://proceedings.neurips.cc/paper\\_files/paper/2022/file/67d57c32e20fd0a7a302cb81d36e40d5-Paper-Conference.pdf](https://proceedings.neurips.cc/paper_files/paper/2022/file/67d57c32e20fd0a7a302cb81d36e40d5-Paper-Conference.pdf).
...
- [36] Dan Hendrycks et al. *Measuring Massive Multitask Language Understanding*. 2021. arXiv: [2009.03300](https://arxiv.org/abs/2009.03300) [cs.GY]. URL: <https://arxiv.org/abs/2009.03300>.
...
- [60] Jiawei Liu et al. “Is Your Code Generated by ChatGPT Really Correct? Rigorous Evaluation of Large Language Models for Code Generation”. In: *Thirty-seventh Conference on NeurIPS*. 2023. URL: <https://openreview.net/forum?id=1qvz610Cv7>.
...
- [65] William Merrill and Ashish Sabharwal. “The Parallelism Tradeoff: Limitations of Log-Precision Transformers”. In: *Transactions of the Association for Computational Linguistics* 11 (2023), pp. 531–545. DOI: [10.1162/tac1\\_a\\_00562](https://doi.org/10.1162/tac1_a_00562). URL: <https://aclanthology.org/2023.tac1-1.31/>.
...
- [73] Piotr Piegok, Róbert Csordás, and Jürgen Schmidhuber. “Mixture of Sparse Attention: Content-Based Learnable Sparse Attention via Expert-Choice Routing”. In: *arXiv preprint arXiv:2505.00315* (2025).
...
- [102] Yaoyu Wang. *Understanding DeltaNet from the Perspective of Inference Frameworks*. May 2025. URL: <https://yywangcs.github.io/DeltaNet-1fefc9fd80580a496f8eb406a496f09>.
...
| Jiezhong Qiu <sup>4</sup>  | Yulun Du                 |
| Bo Pang                    |                          |
...
<sup>4</sup> Hangzhou Institute of Medicine, CAS
...
|                | OJBench (Pass@1)          | 14.2                 | -                  |
|                | Humaneval <sup>+</sup>    | 70.9                 | 46.3               |
|                | MBPP <sup>+</sup>         | 72.4                 | 56.3               |