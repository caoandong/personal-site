_Source: [Aleksa Gordić’s original post](https://www.aleksagordic.com/blog/matmul)._
...
![Figure 1: Model of the NVIDIA Hopper H100 GPU](matmul-assets/h100_model.png)
...
![Figure 2: Memory hierarchy of the H100 (SXM5) GPU](matmul-assets/mem_hierarchy.png)
...
![Figure 3: H100 SXM5 BF16 speed-of-light derivation](matmul-assets/h100_sol.png)
...
![Figure 4: Power throttling reduces clock frequency and lowers the effective “speed of light”](matmul-assets/clk.png)
...
![Figure 5: CUDA Programming Model: threads, warps, blocks, clusters, grids](matmul-assets/cuda_model.png)
...
![Figure 6: CUDA's built-in variables: how threads know where they are](matmul-assets/cuda_model2.png)
...
![Figure 7: Inside a DRAM cell: transistor + capacitor, wordline + bitline](matmul-assets/gmem_dram_cell.png)
...
![Figure 8: GMEM model](matmul-assets/gmem.png)
...
![Figure 9: Effect of access pattern in GMEM](matmul-assets/gmem_example.png)
...
![Figure 10: SMEM model](matmul-assets/smem_pt1.png)
...
![Figure 11: SMEM: good vs. bad access patterns](matmul-assets/smem_pt2.png)
...
![Figure 12: SMEM: multicasting (served in a single cycle)](matmul-assets/smem_pt3.png)
...
![Figure 13: L1 cache model](matmul-assets/l1.png)

*Figure 13: L1 cache model*

Let's go one level deeper and look at the tag stage and data stage in detail:

![Figure 14: Breakdown of k-way set-associative cache organization](matmul-assets/kway.png)
...
![Figure 15: CUDA compilation flow: from CUDA C++ → PTX → SASS](matmul-assets/ptx_sass_hierarchy.png)
...
![Figure 16: Naive CPU matmul example](matmul-assets/cpu_kernel_example.png)
...
![Figure 17: Tile quantization](matmul-assets/tile_quantization.png)
...
![Figure 18: Thread organization in naive matmul kernel](matmul-assets/simple_kernel_logical.png)

*Figure 18: Thread organization in naive matmul kernel*

And the matmul logic itself looks like this:

![Figure 19: Naive matmul kernel](matmul-assets/simple_kernel.png)
...
![Figure 20: Nsight Compute: Occupancy, Waves info](matmul-assets/occupancy_waves.png)
...
![Figure 21: Wave quantization](matmul-assets/wave_quant.png)
...
![Figure 22: PTX code corresponding to naive matmul CUDA kernel](matmul-assets/ptx.png)
...
![Figure 23: SASS code corresponding to naive matmul CUDA kernel](matmul-assets/sass.png)
...
![Figure 24: New logical organization of row2 and col2 variables](matmul-assets/simple_kernel_uncoalesced_logical.png)

*Figure 24: New logical organization of row2 and col2 variables*

And here's what the modified kernel does now:

![Figure 25: Naive kernel with uncoalesced GMEM access](matmul-assets/simple_kernel_uncoalesced.png)
...
![Figure 26: Roofline model](matmul-assets/roofline.png)
...
![Figure 27: Matmul as a sum of partial outer products](matmul-assets/outer.png)
...
![Figure 28: Dot product is equivalent to a sum of partial dot products](matmul-assets/block_tile_equivalence.png)
...
![Figure 29: Arithmetic intensity improves when each thread computes multiple outputs and when tiles approach a square shape](matmul-assets/arithmetic_intensity.png)
...
![Figure 30: High-level structure of the warp-tiling algorithm, also referred to as block tiling.](matmul-assets/warp_tiling_pt1.png)
...
![Figure 31: Loading a chunk of B (GMEM) into Bs (SMEM)](matmul-assets/warp_tiling_pt2_smem_load_b.png)
...
![Figure 32: Loading a chunk of A (GMEM) into As (SMEM)](matmul-assets/warp_tiling_pt2_smem_load.png)
...
![Figure 33: Performing matmul between As and Bs as a series of thread-level outer products (warp-tiling + thread-tiling).](matmul-assets/warp_tiling_pt3_compute.png)
...
![Figure 34: We keep the same high-level structure of the warp-tiling algorithm (block-tiling).](matmul-assets/high_level_block_structure.png)
...
![Figure 35: Swizzling example](matmul-assets/swizzle_example.png)
...
![Figure 36: No bank conflicts when loading rows or columns](matmul-assets/swizzle_property.png)
...
![Figure 37: Swizzle function intuition](matmul-assets/swizzle_function_intuition.png)
...
![Figure 38: Connecting the swizzle function to the matrix swizzle example](matmul-assets/swizzle_apply.png)
...
![Figure 39: Why doing four 64x16 @ 16x64 wgmma calls is equivalent to doing a 64x64 @ 64x64 matmul](matmul-assets/4_wgmma_calls.png)
...
![Figure 40: We are wasting TMA and TC cycles - we can do better](matmul-assets/better_pipeline.png)
...
![Figure 41: More efficient TC/TMA pipeline: producer warp-group streams tiles into a circular buffer; consumer warp-group drains tiles into tensor cores.](matmul-assets/kernel4_pipeline.png)
...
![Figure 42: Two consumer warp groups let us grow the tile from 128x128 -> 128x256 without register spills](matmul-assets/2wgs.png)
...
![Figure 43: Persistent kernels: overlap the output store with incoming loads by launching one long-lived block per SM that processes many tiles.](matmul-assets/overlap_stores.png)
...
![Figure 44: Naïve schedule](matmul-assets/naive_schedule.png)

*Figure 44: Naïve schedule*

Can we do better? Yes—by making the schedule cache-aware:

![Figure 45: Block-wise cache-aware schedule](matmul-assets/blockwise_schedule.png)

*Figure 45: Block-wise cache-aware schedule*

But can we do even better? Surprisingly, yes—by using a space-filling curve:

![Figure 46: Hilbert-curve schedule](matmul-assets/hilbert_schedule.png)
...
![Figure 47: Using thread block clusters to reduce the number of L2/GMEM loads.](matmul-assets/clusters.png)
